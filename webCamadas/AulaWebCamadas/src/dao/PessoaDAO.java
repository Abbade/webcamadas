...
public class PessoaDAO implements InterfacePessoaDAO {
	...
	public Boolean deletar(int _id) throws SQLException {
		...
		EnderecoDAO daoEndereco = new EnderecoDAO(this.conexao);
		List<Endereco> listaEnderecos = daoEndereco.buscarEnderecosDaPessoa(_id);
...
//		}
	...
	public Pessoa buscarPorID(int _id) throws SQLException {
		...
		if (rs.next()) {
			...
			EnderecoDAO daoEndereco = new EnderecoDAO(this.conexao);
			List<Endereco> enderecos = daoEndereco.buscarEnderecosDaPessoa(p.getId());
...
//			p.setDtCadastro(rs.getDate(4));
			...
			EnderecoDAO daoEndereco = new EnderecoDAO(this.conexao);
			List<Endereco> enderecos = daoEndereco.buscarEnderecosDaPessoa(id);
			
			//p.setEnderecos(enderecos);
			
			listaPessoas.add(new Pessoa(id, nome, tel, data, enderecos));//VOLTAR AQUI
		}
